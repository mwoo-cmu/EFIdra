...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = efidraFileSystem.MyFileSystemFactory.class)
public class efidraFileSystem implements GFileSystem {
	 ...
	 */
	public efidraFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + efidraFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<efidraFileSystem>,
			GFileSystemProbeByteProvider {

		@Override
		public efidraFileSystem create(FSRLRoot targetFSRL,
				ByteProvider byteProvider, FileSystemService fsService, TaskMonitor monitor)
				throws IOException, CancelledException {

			efidraFileSystem fs = new efidraFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
